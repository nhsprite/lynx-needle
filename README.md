...
## Prerequisites: NAPI requires Lynx ≥ 4.3.0 (nightly) or a source build

The Lynx NAPI addon flow (`requireNodeAddon` /
`__lynx_node_addon_exports__`) requires the runtime to be built with
`enable_napi_binding=true`, otherwise `RuntimeLifecycleListener.onRuntimeAttach`
never fires and no `napi_env` reaches the host.

- **Lynx 4.3.0 nightly snapshots are the first published artifacts with NAPI
  binding compiled in** (verified end-to-end on a real device with
  `4.3.0-nightly.202609010611.167.g17ad2906-SNAPSHOT`). They live in the Maven
  Central **snapshot** repository, not the release repo:
  ...
  // app/build.gradle
  implementation 'org.lynxsdk.lynx:lynx:4.3.0-nightly.202609010611.167.g17ad2906-SNAPSHOT'
  ```

  Note the `RuntimeLifecycleListener` interface there is the two-arg
  `onRuntimeAttach(long napiEnv, String runtimeType)`; older 3.9.x-era
  single-arg listeners silently never match.
- **Older releases do not work.** We verified the `liblynx.so` shipped by
  every release-repo artifact (3.9.1, 3.9.1-dev, 4.0.1, 4.0.0 / 4.1.0
  nightlies, plain and `-dev`) was built with `enable_napi_binding=false`.
- **Source build** remains an option: pass
  `enable_napi_binding=true enable_lepusng_worklet=true` (the gn args
  LynxExplorer passes, see
...
On iOS the published pods are still not sufficient: the `Lynx` podspec defines
`ENABLE_NAPI_BINDING=1`, but (a) the PrimJS and LynxWeakNodeAPI pods ship
colliding public `js_native_api*.h` headers (the example's Podfile works around
this with forwarding headers) and (b) the `Lynx` pod's NAPI sources reference
`third_party/napi/include/*` headers that are not packaged in the pod. A
working iOS NAPI build requires building Lynx from source (as LynxExplorer
does).
