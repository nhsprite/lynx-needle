...
public class LynxNodeAPIModule extends LynxContextModule {
  ...
  static {
    try {
      ...
      Log.i(TAG, "native loader lynx_napi_addon_loader loaded");
  ...
  public static void putEnv(LynxContext token, long napiEnv) {
    if (token != null && napiEnv != 0) {
      ...
      Log.i(TAG, "putEnv: runtimeId=" + runtimeId + " napiEnv=" + napiEnv);
      if (runtimeId != null) {
  ...
  public void requireNodeAddon(String addonName) {
    Log.i(TAG, "requireNodeAddon called: " + addonName);
    if (!sNativeAvailable) {
      Log.e(TAG, "Native addon loader unavailable; ignore requireNodeAddon: " + addonName);
      return;
    }
