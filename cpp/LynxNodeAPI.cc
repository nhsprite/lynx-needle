...
#define LNA_LOGD(...) \
  __android_log_print(ANDROID_LOG_DEBUG, "LynxNodeAPI", __VA_ARGS__)
#define LNA_LOGE(...) \
  __android_log_print(ANDROID_LOG_ERROR, "LynxNodeAPI", __VA_ARGS__)
#else
#define LNA_LOGD(...) ((void)0)
#define LNA_LOGE(...) std::fprintf(stderr, __VA_ARGS__)
#endif
#if defined(_WIN32)
#include <windows.h>
#endif
...
// lynx-needle: headers come from the @lynx-js/weak-node-api npm package
// (repo-root node_modules), wired via CMake include path.
#include "node_api.h"
#ifdef USE_WEAK_SUFFIX_NAPI
...
void LynxNodeAPI::RequireNodeAddon(void* napi_env_ptr,
  ...
  if (napi_env_ptr == nullptr) {
    LNA_LOGE("RequireNodeAddon: null napi_env for addon '%s'\n",
             addon_name.c_str());
    return;
  }
  LNA_LOGD("RequireNodeAddon: loading addon '%s' (env=%p)\n",
           addon_name.c_str(), napi_env_ptr);
  ...
  NodeAddon addon;
  {
    ...
    if (inserted) {
      if (!LoadNodeAddon(cached_addon, addon_name)) {
        ...
        LNA_LOGE("RequireNodeAddon: LoadNodeAddon failed for '%s'\n",
                 addon_name.c_str());
        return;  // Load failed
      }
    }
  ...
  bool ok = InitializeNodeModule(napi_env_ptr, addon);
  if (ok) {
    LNA_LOGD("RequireNodeAddon: addon '%s' published\n", addon_name.c_str());
  } else {
    LNA_LOGE("RequireNodeAddon: InitializeNodeModule FAILED for '%s'\n",
             addon_name.c_str());
  }
}
...
bool LynxNodeAPI::LoadNodeAddon(NodeAddon& addon,
  ...
  for (const auto& path : candidates) {
...
#else
      last_error = dlerror();
      LNA_LOGE("LoadNodeAddon: dlopen('%s') failed: %s\n", path.c_str(),
               last_error != nullptr ? last_error : "unknown");
#endif
      continue;
    }

    LNA_LOGD("LoadNodeAddon: dlopen('%s') OK\n", path.c_str());
    ...
    LNA_LOGE("LoadNodeAddon: '%s' missing napi_register_module_v1\n",
             path.c_str());
...
bool LynxNodeAPI::InitializeNodeModule(void* env_ptr, NodeAddon& addon) {
  ...
  if (status != napi_ok) {
    LNA_LOGE("InitializeNodeModule: napi_create_object failed: %d\n", status);
    return false;
  }
  ...
  if (status != napi_ok) {
    LNA_LOGE("InitializeNodeModule: napi_get_global failed: %d\n", status);
    return false;
  }
  ...
  if (type != napi_object) {
    ...
    if (status != napi_ok) {
      LNA_LOGE("InitializeNodeModule: recreate exports_obj failed: %d\n",
               status);
      return false;
    }
    status = napi_set_named_property(env, global, "__lynx_node_addon_exports__",
                                     exports_obj);
    if (status != napi_ok) {
      LNA_LOGE("InitializeNodeModule: set __lynx_node_addon_exports__ "
               "failed: %d\n",
               status);
      return false;
    }
  ...
  if (status != napi_ok) {
    LNA_LOGE("InitializeNodeModule: publish '%s' failed: %d\n",
             addon.generatedName.c_str(), status);
  }
